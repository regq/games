...
10. Skins are data, and `skin.json` carries the version gate (2026-09-23). A game's look is `skin.json`: `palette` (named slot → `#rgb`/`#rrggbb`), `sprites` (slot → PNG data URI), `sfx` (slot → WAV or OGG data URI), and `engine` (a `>=X.Y.Z` the loader enforces against the game's own version). **v1 limits, refused not truncated:** at most 12 sprite slots, each ≤ 32 KB; each sound ≤ 64 KB; PNG, WAV and OGG data URIs only. **SVG is excluded** until a sanitizer exists — it carries script and external references, and a skin is the one file a stranger will eventually supply. A skin that fails validation is refused whole and the built-in is used; a skin whose `engine` is newer than the game is refused the same way, with one line saying so. The engine reads slots, never literals. The `engine` key moved here from `levels.json`, where nothing read it and nothing could: content and engine ship in the same commit under one tag, so they cannot diverge — a skin is the file that can.

11. A flag is scored by LIFT or by TARGET, and the target form is not judged on polarity (2026-09-23). A metric may carry a level: `<metric>@L<n>`. **Lift form** (unchanged): `expect` is a positive magnitude of improvement, and the direction comes from the metric — up for `win_rate`, `level_win_rate`, `session_len_s`, `thumbs_up_rate`; down for `fail_per_session`, `rage_quit_rate`. **Target form**: `metric <m>@L<n> target <v> tolerance <t>`. The canary wins when it lands **closer to the target than control by at least the tolerance**, reverts when it lands farther by at least the tolerance, and is otherwise too close to call. It exists because the lift form can only say "more is better" or "less is better": a change asked to put a number **on** a value — 12% of runs reaching L3, not more and not less — has no good direction, and scoring it by polarity reverts a flag for moving the number the way it was asked to. A target-form flag's verdict never reads `lift` or `expect`. Both forms obey the same window, the same ≥ 30 sessions per arm, and the same three-window limit. A flag carries one form or the other, never both. `ctx.run` (a per-session run counter) and `ctx.seed` (the run's PRNG seed) make a run countable and reproducible, and `level_win_rate@L<n>` is runs that won that level over runs.

12. `kind: page`, whose levels are doors (2026-09-23). A page is a spoke with no loop. Its mapping: **`level` = a door**, numbered by a stable id from the door's **slug, never its position** — the "door order" increment reorders the page, and a position-keyed level would compare two different doors across one window. **`level_win`** = that door clicked. **`level_fail`** = at session end, if no door was clicked *and* the session lasted ≥ 20 s, one per **live** door that was on screen. A muted door can never fail, because it could never be clicked. Under 20 s nothing is emitted: that is a bounce, visible as a session with zero wins, and blaming a door for a page nobody read would poison the fail cliff. `purchase` fires **in addition to** `level_win` on a money door — the win says a door earned a click, the purchase says a money door opened. `ugc_publish` / `ugc_play` are unused and are not faked. Consequences, accepted: several fails land in one session, so session-level `fail_per_session` is inflated for a page and every page template proves on a per-door metric instead; `quit_point` is meaningless (a page emits its fails together, so "the level they quit on" is whichever sorted last) and is not ranked; the PAIN line for a page reports the worst door and click-through in place of the level clauses. **A game template must never fire on a page** — each names a band, a gauge or a hold, none of which a page has — so templates are keyed by kind and an unknown kind proposes nothing.
